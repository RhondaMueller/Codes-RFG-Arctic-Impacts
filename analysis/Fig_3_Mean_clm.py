...
def rfg_mean_clm(RFG1, RFG2, RFG3, cam02): ## cam02: "CCT", "MSB", "" (put nothing for clm)
    ...
    RFG_mean = []
    for k in range(0, len(RFG1_roll)):
        RFG_mean.append((RFG1_roll[k] + RFG2_roll[k] + RFG3_roll[k]) / 3)
...
def year_roll_mean(RFG):
    ...
    while i in range(0, len(RFG)):
        RFG_mean.append(np.mean(RFG[i+5:i+8]))
        i = i + 12
    return(RFG_mean)
...
def month_roll_mean(RFG, month):
    ...
    i = 0
    while i in range(0, len(RFG)):
        RFG_mean.append(np.mean(RFG[i + month]))
        i = i + 12
